# Payment Processing Service

Сервис обработки платежей с учётом комиссии и интеграцией с внешним платёжным шлюзом (имитация).

Проект реализуется как тестовое задание и спроектирован с упором на:

* чистую архитектуру
* масштабируемость
* надёжность
* разделение ответственности между слоями

---

## 🧠 Общая идея

Сервис позволяет:

* создавать и хранить пользователей с базовой бизнес-логикой
* принимать платежи (депозит) и вычитать средства (withdraw) с учётом комиссии
* хранить историю платежей и комиссий

API остаётся отзывчивым даже при высокой нагрузке, тяжёлые операции можно будет выносить в отдельные воркеры (пока без Celery).

---

## 🏗 Архитектура

Проект построен по **слоистой архитектуре (Clean Architecture)**:

```
API → Application → Domain
↓
Infrastructure
```

### Слои:

* **Domain** — бизнес-сущности и правила (не зависит от фреймворков)
* **Application** — use cases и сценарии обработки
* **Infrastructure** — БД, репозитории, внешние сервисы
* **API** — HTTP-интерфейс (FastAPI)
* **Workers** — фоновая обработка (пока не реализовано)

---

## 📁 Структура проекта

```
app/
├── api/
│   └── v1/
│       ├── __init__.py
│       ├── payments.py          # депозиты и вычеты
│       ├── users.py             # создание пользователя
│       ├── health.py
│       └── schemas/
│           ├── users.py         # схемы для User
│           └── payment.py       # схемы для депозитов и withdraw
├── application/
│   ├── use_cases/
│   │   ├── create_user.py
│   │   ├── deposit_balance.py
│   │   └── withdraw_balance.py
│   ├── dto/
│   │   └── payment.py           # DTO для депозитов и withdraw
│   └── interfaces/
├── domain/
│   ├── entities/
│   │   └── user.py
│   ├── enums/
│   └── exceptions.py
├── infrastructure/
│   ├── db/
│   ├── repositories/
│   │   ├── user.py
│   │   ├── payment.py
│   │   └── transaction.py
│   ├── payment_gateway/
│   │   ├── base.py
│   │   └── http.py
│   ├── queue/
│   └── cache/
├── workers/                     # пока пусто
├── core/
│   ├── settings.py
│   ├── logging.py
│   └── dependencies.py
└── main.py
```

---

## 🔌 API versioning

Все HTTP-эндпоинты находятся под версией:

```
/api/v1/*
```

Версия агрегируется в `api/v1/__init__.py`, что позволяет:

* подключать API одной строкой
* легко добавлять новые роутеры
* в будущем безболезненно добавить `v2`

---

## ⚙️ Конфигурация

Конфигурация приложения осуществляется через **environment variables** и загружается с помощью `pydantic-settings`.

Пример `.env`:

```env
DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/payments
REDIS_URL=redis://localhost:6379/0
PAYMENT_GATEWAY_URL=http://localhost:8001
TRANSACTION_FEE=2
```

---

## 🚀 Запуск (на текущем этапе)

### Установка зависимостей

```bash
poetry install
```

### Запуск приложения

```bash
uvicorn app.main:app --reload
```

### Проверка

* Swagger UI: `http://localhost:8000/docs`
* Health-check: `GET /api/v1/health`
* Создание пользователя: `POST /api/v1/users/`
* Депозит: `POST /api/v1/payments/deposit`
* Вычет средств: `POST /api/v1/payments/withdraw`

---

## 📌 Текущий статус проекта

✅ Реализовано:

* базовая структура проекта
* слоистая архитектура
* versioned API
* health-check
* конфигурация через env
* **Создание пользователя**:

  * use case `create_user`
  * схемы пользователя в `schemas/users.py`
  * сущность `User` в `domain/entities/user.py`
  * репозиторий `UserRepository`
  * POST-эндпоинт `/api/v1/users/`
* **Пополнение баланса (депозит)**:

  * use case `deposit_balance`
  * DTO `DepositDTO`
  * репозитории `PaymentRepository`, `TransactionRepository`
  * эндпоинт `/api/v1/payments/deposit`
  * расчёт комиссии 2%
  * обновление баланса пользователя
  * запись платежа и транзакции в БД
  * атомарность через транзакцию SQLAlchemy
* **Вычет баланса (withdraw)**:

  * use case `withdraw_balance`
  * DTO `WithdrawDTO`
  * репозитории `PaymentRepository`, `TransactionRepository`
  * эндпоинт `/api/v1/payments/withdraw`
  * проверка баланса
  * расчёт комиссии 2%
  * атомарное обновление баланса и запись транзакции

⏳ В процессе:

* Фоновые воркеры (Celery)
* Retry и Dead Letter Queue
* Docker Compose

---

## 🧪 Тестирование

Тесты будут добавляться по мере реализации бизнес-логики
(юнит-тесты для domain и application слоёв).

---

## 📄 Примечание

Проект разрабатывается итеративно.
README будет обновляться по мере добавления функциональности.
